...
import { IconBadge, LeafIcon, CupIcon } from "../shared/ui/Badge";
...
export default function CoffeePage() {
  const { t, i18n } = useTranslation(["coffee", "common"]);
  const { addToCart } = useCart();
  const items = useMemo(() => Object.values(coffeeCatalog || {}), []);
  ...
  const getCoffeeDetailPath = (slugOrId) =>
    `/${lang2}/${routeMap.coffeeDetail[lang2]}/${slugOrId}`;
    ...
    <main className="pt-32 px-4 py-8 max-w-6xl mx-auto [color-scheme:light] dark:[color-scheme:dark]">
      <div className="flex items-center justify-between gap-3 mb-4">
        <h1 className="text-3xl font-bold text-green-800 dark:text-green-400">
          {t("title", { defaultValue: "☕ Coffee Selection" })}
        </h1>
        <CurrencySelect />
      </div>
      ...
      <p className="text-slate-600 dark:text-slate-400 mb-6">
        {t("subtitle", {
          defaultValue:
            "Single-origin coffees from Africa's storied highlands — bright florals, sun-ripe berries, cocoa-soft finishes.",
        })}
      </p>
      ...
      <div className="mb-6">
        <input
          ...
          placeholder={t("searchPlaceholder", {
            defaultValue: "Search single-origins or notes…",
          })}
      ...
      <section className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {pageItems.map((p) => {
          const basePrice = p.prices?.SEK ?? p.price ?? 0;
          const detailHref = getCoffeeDetailPath(p.slug || p.id);
            ...
            >
              ...
              <div className="flex items-start justify-between gap-3 mt-3">
                <h2 className="text-xl font-semibold text-green-700 dark:text-green-300">
                  {nameT(t, "products.coffee", p.id, p.name)}
                </h2>
                <div className="flex items-center gap-2">
                  ...
                  <IconBadge tooltip={t("badges.caffeinated", { defaultValue: "Caffeinated" })}>
                    <CupIcon />
                  </IconBadge>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 mb-3">
                {descT(t, "products.coffee", p.id, p.description)}
              </p>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-16 px-2 py-1 border rounded dark:bg-slate-900 dark:border-slate-700"
                  />
                  <button
                    onClick={() => add(p)}
          ...
          >
            ...
            <div className="flex items-start justify-between gap-2 mb-2">
              <h2 className="text-xl font-bold">
                {nameT(t, "products.coffee", selected.id, selected.name)}
              </h2>
              <LikeButton productId={selected.id} size="sm" />
            </div>

            <p className="text-sm mb-3">
              {descT(t, "products.coffee", selected.id, selected.description)}
            </p>
            ...
            <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
              <div className="flex items-center gap-2">
                ...
                {/* ✅ Learn more (modal) */}
                <Link
                  to={getCoffeeDetailPath(selected.slug || selected.id)}
                  className="text-sm text-green-700 hover:underline dark:text-green-300 whitespace-nowrap"
                >
                  {t("learnMore", { ns: "common", defaultValue: "Learn more" })}
                </Link>
