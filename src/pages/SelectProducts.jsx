...
// ✅ Helper functions
const getTranslatedLabel = (product, category, t) => {
  if (category === "agro") {
...
const getTranslatedDescription = (product, category, t) => {
  if (category === "agro") {
...
export default function SelectProducts() {
  ...
  const [showAgro, setShowAgro] = useState(false); // ✅ Toggle Agro display

  const tLabels = {
    title: t("order.title", { defaultValue: "Order products" }),
    currencyLabel: t("order.currencyLabel", { defaultValue: "Currency" }),
    currencyWarning: t("order.currencyWarning", {
      defaultValue: "Note: Swish and Bankgiro only support SEK.",
    }),
    addedToCart: t("order.addedToCart", {
      defaultValue: "Item added to cart!",
    }),
    proceedToCheckout: t("order.proceedToCheckout", {
      defaultValue: "Proceed to checkout",
    }),
    categoryLabel: t("order.categoryLabel", {
      defaultValue: "Filter by category:",
    }),
    searchPlaceholder: t("order.searchPlaceholder", {
      defaultValue: "Search by product name...",
    }),
    contactToOrder: t("order.contactToOrder", {
      defaultValue: "Contact to order",
    }),
  };
  ...
  const productGroups =
    selectedCategory === "all"
      ? Object.entries(productImages).filter(
          ([key]) => showAgro || key !== "agro"
        )
      : selectedCategory === "agro" && !showAgro
      ? []
      : [[selectedCategory, productImages[selectedCategory]]];

  const getTranslatedCategoryName = (key) => {
    if (key === "cosmetics")
      return t("categories.cosmetics", { defaultValue: "Cosmetic Products" });
    if (key === "food")
      return t("categories.food", { defaultValue: "Healing Botanicals" });
    if (key === "agro")
      return t("categories.agro", {
      ...
      <div className="max-w-5xl mx-auto p-6 bg-white dark:bg-gray-800 rounded shadow">
        ...
        <div className="mb-4">
          <label className="inline-flex items-center space-x-2 text-sm text-gray-700 dark:text-gray-200">
            <input
              type="checkbox"
              checked={showAgro}
              onChange={(e) => setShowAgro(e.target.checked)}
              className="accent-green-600"
            />
            <span>
              {t("order.showAgro", { defaultValue: "Show Agro Products" })}
            </span>
          </label>
        </div>
        ...
        <form className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          ...
          <div>
            ...
            >
              <option value="all">
                {t("categories.all", { defaultValue: "All Categories" })}
              </option>
              {Object.keys(productImages)
                .filter((category) => category !== "agro")
                .map((category) => (
          ...
          <div>
            <label className="block mb-1">
              {t("order.searchLabel", { defaultValue: "Search" })}
            </label>
            <input
              type="text"
                ...
                .map((product, idx) => (
                  <ProductCard
                    key={`${category}-${subgroup}-${
                      product.slug || product.id || product.label || idx
                    }`}
                    category={category}
                    product={{
                      ...product,
                      label: getTranslatedLabel(product, category, t),
                    ...
                    contactLabel={tLabels.contactToOrder}
                    ...
                    onAddToCart={(e) => {
                      ...
                      setToastMessage(
                        `✅ ${product.label} ${tLabels.addedToCart}`
                      );
        ...
        {formData.products.length > 0 && (
          <div className="bg-gray-100 dark:bg-gray-700 p-4 rounded mb-4">
            <h3 className="font-semibold mb-2 text-gray-800 dark:text-gray-200">
              {t("order.selectedItems", { defaultValue: "Selected items" })}
            </h3>
            <ul className="text-sm text-gray-700 dark:text-gray-100">
              {formData.products.map((label) => (
                <li key={label}>
                  {t(`products.${label}`, { defaultValue: label })} ×{" "}
                  {quantities[label] || 1}
                </li>
              ))}
            </ul>
