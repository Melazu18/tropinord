// src/pages/OilsProductDetail.jsx
...
export default function OilsProductDetail() {
  const { slug } = useParams();
  const { t } = useTranslation(["oil", "common", "buttons", "products.oils"]);
  ...
  const product = useMemo(() => {
    return (
      oilCatalogData?.[slug] ||
      Object.values(oilCatalogData || {}).find((x) => x.slug === slug) ||
      null
    );
  }, [slug]);
  ...
  const translatedName = nameT(t, "products.oils", product.id, product.name);
  ...
  return (
    <main className="pt-24 px-4 py-10 max-w-5xl mx-auto">
      <div className="mb-4">
        <BackButton fallbackRouteKey="oils" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-lg shadow p-6">
          ...
          <p className="text-slate-600 dark:text-slate-300 mt-2">
            {descT(t, "products.oils", product.id, product.description)}
          </p>
          ...
          {product.ingredients && (
            <div className="mt-6">
              <h2 className="text-lg font-semibold">
                {t("modal.ingredientsHeading", { ns: "oil", defaultValue: "Ingredients" })}
              </h2>
      ...
      <section className="mt-6 bg-white dark:bg-gray-900 rounded-lg shadow p-6">
        <h2 className="text-xl font-bold">
          {t("detail.reviews", { ns: "oil", defaultValue: "Reviews" })}
        </h2>
